---
name: jetson-deployment-agent
description: Analyze and automate deployment workflows for NVIDIA Jetson devices by parsing tutorial/wiki requirements, collecting Jetson runtime facts, checking JetPack 5.x and 6.x compatibility, mapping dependencies to apt/pip/conda, generating adjusted install commands, and running guided deployment with safety confirmations. Use when a user asks to deploy a project on Jetson from an external tutorial link, validate hardware/software compatibility, resolve version conflicts, or produce reproducible Jetson setup and run steps.
---
...
- `scripts/collect_jetson_facts.sh`
- `scripts/extract_tutorial_requirements.py`
- `scripts/analyze_compatibility.py`
- `scripts/generate_deploy_plan.py`
- `scripts/execute_guided_plan.sh`
- `references/jetpack_compatibility_matrix.json`
- `references/jetpack-compatibility-matrix.md`
- `references/dependency-mapping-rules.md`
- `references/tutorial-parsing-rules.md`
...
1. Run:
```bash
python3 scripts/extract_tutorial_requirements.py \
...
1. Local collection:
```bash
bash scripts/collect_jetson_facts.sh --local --output "<facts.json>"
```
2. Remote collection:
```bash
bash scripts/collect_jetson_facts.sh \
  --host "<ip-or-host>" \
  --user "<ssh-user>" \
  --port 22 \
  --output "<facts.json>"
...
1. Run:
```bash
python3 scripts/analyze_compatibility.py \
  --facts "<facts.json>" \
...
1. Generate plan:
```bash
python3 scripts/generate_deploy_plan.py \
...
1. Execute plan:
```bash
bash scripts/execute_guided_plan.sh \
